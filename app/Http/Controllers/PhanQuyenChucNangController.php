...
class PhanQuyenChucNangController extends Controller
{
    public function get_list_chuc_nang() {
        ...
        $nhom_cn_user = DB::select('SELECT pq.id_nhom_chuc_nang, pq.ten_nhom, pq.icon, pq.thu_tu_nhom FROM phan_quyen as pq WHERE pq.id_nhom_nguoi_dung ='.$nhom_ng->id_nhom_nguoi_dung.' GROUP BY pq.id_nhom_chuc_nang, pq.ten_nhom, pq.icon ORDER BY pq.thu_tu_nhom ASC');
        $list_chuc_nang = [];
        if($nhom_cn_user) {
